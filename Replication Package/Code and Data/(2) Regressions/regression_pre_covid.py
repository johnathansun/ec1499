...
Python replication of regression_pre_covid.do
...
Key difference from regression_full.py:
- Wage equation (gw): estimated on pre-COVID sample (period <= 2019 Q4)
- Price equation (gcpi): estimated on FULL sample (because of shortage variable)
- cf1 equation: estimated on pre-COVID sample (period <= 2019 Q4)
- cf10 equation: estimated on pre-COVID sample (period <= 2019 Q4)
- No dummy variables for Q2/Q3 2020
...
# Output Location
# output_dir = Path("../Replication Package/Code and Data/(2) Regressions/Output Data (Restricted Sample)")
output_dir = Path("/Users/johnathansun/Documents/ec1499/Replication Package/Code and Data/(2) Regressions/Output Data Python (Pre Covid Sample)")
...
keep_vars = ['period', 'gcpi', 'vu', 'gw', 'magpty', 'grpe', 'grpf', 'cf1', 'cf10',
             'shortage', 'diffcpicf', 'CPIENGSL', 'CPIUFDSL']
...
#*******************************WAGE EQUATION (gw)********************************
# Note: Estimated on PRE-COVID sample (period <= 2019 Q4), no dummy variables
print("\n" + "="*80)
print("ESTIMATING WAGE EQUATION (gw) - PRE-COVID SAMPLE")
print("="*80)
...
# Build X matrix for wage equation (NO dummy variables in pre-covid version)
...
# Filter to pre-COVID sample for estimation (period <= 2019 Q4)
...
# Indices: 0-3 (L1-L4 gw), 4-7 (L1-L4 cf1)
...
with pd.ExcelWriter(output_dir / 'eq_coefficients_restricted_python.xlsx', engine='openpyxl', mode='w') as writer:
...
bb1 = sum(results_wage.params[const_offset + 9 + i] for i in range(0, 4))  # L1-L4 vu
cc1 = sum(results_wage.params[const_offset + 13 + i] for i in range(0, 4))  # L1-L4 diffcpicf
...
# Calculate R-squared (correlation squared) - PRE-COVID sample only
...
with pd.ExcelWriter(output_dir / 'summary_stats_restricted_python.xlsx', engine='openpyxl', mode='w') as writer:
    summary_wage.to_excel(writer, sheet_name='gw', index=False)

print(f"R-squared (pre-COVID): {r2_wage:.7f}")
print(f"Number of observations: {results_wage.nobs}")


#*******************************PRICE EQUATION (gcpi)*****************************
# Note: Estimated on FULL sample (because of shortage variable)
print("\n" + "="*80)
print("ESTIMATING PRICE EQUATION (gcpi) - FULL SAMPLE")
print("="*80)
...
print("Saving coefficients...")
with pd.ExcelWriter(output_dir / 'eq_coefficients_restricted_python.xlsx', engine='openpyxl', mode='a') as writer:
...
# Calculate R-squared - FULL sample (from 1990)
...
with pd.ExcelWriter(output_dir / 'summary_stats_restricted_python.xlsx', engine='openpyxl', mode='a') as writer:
...
# Note: Estimated on PRE-COVID sample (period <= 2019 Q4)
print("\n" + "="*80)
print("ESTIMATING 1-YEAR EXPECTATIONS EQUATION (cf1) - PRE-COVID SAMPLE")
print("="*80)
...
# Calculate R-squared - PRE-COVID sample only
...
print(f"R-squared (pre-COVID): {r2_cf1:.7f}")
...
# Note: Estimated on PRE-COVID sample (period <= 2019 Q4)
print("\n" + "="*80)
print("ESTIMATING 10-YEAR EXPECTATIONS EQUATION (cf10) - PRE-COVID SAMPLE")
print("="*80)
...
print(f"R-squared (pre-COVID): {r2_cf10:.7f}")
...
# Export to Excel
output_file = output_dir / 'eq_simulations_data_restricted_python.xlsx'
...
print("  - eq_coefficients_restricted_python.xlsx (4 sheets: gw, gcpi, cf1, cf10)")
print("  - summary_stats_restricted_python.xlsx (4 sheets: gw, gcpi, cf1, cf10)")
print("  - eq_simulations_data_restricted_python.xlsx (full dataset with predictions)")
print("\nKey differences from full sample regression:")
print("  - gw, cf1, cf10: estimated on pre-COVID sample (<=2019 Q4)")
print("  - gcpi: estimated on FULL sample (due to shortage variable)")
print("  - Out-of-sample predictions generated for 2020+ periods")
print("  - No dummy variables for Q2/Q3 2020")
print("\n")
