%% load data and estimate VAR, no debt in VAR

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Load data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mdata = xlsread('../RawData/MaindatafileA_Feb2021_longsample.xlsx', 'VAR_international', 'a20:am93'); % 1947-2020
mdata0 = xlsread('../RawData/MaindatafileA_Feb2021_longsample.xlsx', 'VAR_international', 'a19:am19'); % 1946
...
% global variables
inflation_g = log(1 + mdata(Tstart:Tend, 37));
rpcgdpgr_g = log(1 + mdata(Tstart:Tend, 38));
totequityret_g = log(1 + mdata(Tstart:Tend, 39));
...
pi0_g = mean(inflation_g);
x0_g = mean(rpcgdpgr_g);
eret0 = mean(totequityret_g);
...
mu_m = nanmean(divgrm); % mu_m is growth in dividend-gdp ratio
...
tts(1) = taxrevgdp(1) * 0.79386; % 1947 value for nominal tax revenue/nominal GDP; this is annualized per capita
...
gdebt = xlsread('../RawData/nominal_aggr_debt_1946_2020_annual.xlsx', 'Sheet1', 'd2:d76')'; % 1947 - 2020
...
dt = deltalogtau - tau0;
pd = pdm - A0m;
infl_g = inflation_g - pi0_g;
x_g = rpcgdpgr_g - x0_g;
equityret = totequityret_g -eret0;
...
X2(:, pdpos) = pd;
X2(:, inflglobalpos) = infl_g;
X2(:, gdpglobalpos) = x_g;
X2(:, equityretpos) = equityret;
...
I_pi_g = I(:, inflglobalpos);
I_gdp_g = I(:, gdpglobalpos);
I_equityreg = I(:, equityretpos);
...
for i = [inflpos y1pos ysprpos gdppos divgrpos pdpos inflglobalpos gdpglobalpos equityretpos dtpos dgpos]
...
Sigma = cov(eps(:, [inflpos y1pos ysprpos gdppos divgrpos pdpos inflglobalpos gdpglobalpos equityretpos dtpos dgpos]));
...
Sig([inflglobalpos gdpglobalpos equityretpos], [inflpos y1pos ysprpos gdppos divgrpos pdpos inflglobalpos gdpglobalpos equityretpos]) = tmp(7:9, 1:9);
Sig(dtpos, [inflpos y1pos ysprpos gdppos divgrpos pdpos inflglobalpos gdpglobalpos equityretpos dtpos]) = tmp(10, 1:10);
Sig(dgpos, [inflpos y1pos ysprpos gdppos divgrpos pdpos inflglobalpos gdpglobalpos equityretpos dtpos dgpos]) = tmp(11, 1:11);
...
eps = [eps(:, [inflpos y1pos ysprpos gdppos divgrpos]), eps(:, divgrpos), ...
                eps(:, [inflglobalpos gdpglobalpos equityretpos]), ...
