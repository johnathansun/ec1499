function obj = solve_step2(param, param0, N, T, Psi, Sig, I_pi, I_gdp, I_y1, I_yspr, I_pdm, I_divgrm, divgrpos, I_pi_g, I_gdp_g, I_equityreg, ...
    ...
    %% Match Stock PD Ratio
    penalty = 2e4;
    FF_new = penalty * nanmean((exp(A0m + I_pdm' * X2t')' - PDm_model) .^ 2);
    FF = [FF FF_new];

    %% Match Global Stock Risk Premium
    penalty = 1e6;
    model_globalerp = ((I_equityreg)' * Sig * L0 - 0.5 * ((I_equityreg)' * Sig) * ((I_equityreg)' * Sig)') * 100;
    data_globalerp = (eret0 - y0nom_1) * 100;
    FF_new = penalty * (data_globalerp - model_globalerp) .^ 2;
    FF = [FF FF_new];
    penalty = 1e5;
    model_globalerp_cond = ((I_equityreg)' * Sig * (L1 * X2t')) * 100;
    data_globalerp_cond = (I_equityreg' * Psi * X2t') * 100;
    FF_new = penalty * mean((data_globalerp_cond - model_globalerp_cond) .^ 2);
    FF = [FF FF_new];
    ...
    % % Remove wiggles in bond yields
    % for i = 1:length(lookup_horizon)
    %     tt = lookup_horizon(i);
    %     nombondrp = 100*abs((-Api(tt+50)/(tt+50) + Api(tt)/(tt)) - (-Api(tt)/tt + Api(tt-50)/(tt-50)));
    %     FF_new    = penalty*abs(min(1 - nombondrp,0));
    %     FF        = [FF FF_new];
    % end
