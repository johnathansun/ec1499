...
parfor iloop = 1:nloop
    ...
    while (val >= 1)
        ...
        erp = erp0 + (1 - k1m) * (- I_pdm' * X2t' + (I_pi + I_gdp + I_divgrm - I_y1 - I_yspr)' * Psi1 * inv(eye(N) - k1m * Psi1) * X2t');
        DRM = (I_y1 + I_yspr)' * Psi1 * inv(eye(N) - k1m * Psi1) * X2t' + (erp - erp0) / (1 - k1m);
    ...
    DR = (I_y1 + I_yspr)' * Psi1 * inv(eye(N) - k1x * Psi1) * X2t';
    ...
    if (mean(abs(s1)) > 100)
        iloop
        tmp1(iloop, :) = init_index;
        tmp2(iloop, :) = shocks_index;
    end
...
f.PaperSize = [6 4];
print(['../figures/cs_global_', num2str(output_rp * 1000)], '-dpdf', '-fillpage');
