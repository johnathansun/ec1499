%% Load data and estimate VAR, with debt in VAR
...
date = mdata(Tstart:Tend, 1); % 1947-2019
...
X2(:, ddpos) = dd;
X2(:, coint_debt) = log(dts) - mean(log(dts));
...
X2t(:, coint_debt) = log(debtgdp) - mean(log(debtgdp));
...
Sigma = cov(eps(:, [inflpos y1pos ysprpos gdppos divgrpos pdpos dtpos dgpos ddpos]));
...
Sig(ddpos, [inflpos y1pos ysprpos gdppos divgrpos pdpos dtpos dgpos ddpos]) = tmp(9, 1:9);

Sig(coint_div, :) = Sig(divgrpos, :);
Sig(coint_tax, :) = Sig(dtpos, :);
Sig(coint_spending, :) = Sig(dgpos, :);
Sig(coint_debt, :) = Sig(ddpos, :);
