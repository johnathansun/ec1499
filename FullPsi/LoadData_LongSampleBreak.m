...
%% Load data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mdata = xlsread('../RawData/MaindatafileA_Feb2021_longsample.xlsx', 'VAR2', 'a3:aj93'); % 1930-2020
mdata0 = xlsread('../RawData/MaindatafileA_Feb2021_longsample.xlsx', 'VAR2', 'a2:aj2'); % 1929

option.figure = 0;

Tstart = 1; % start sample in 1930
Tend = 91; % end sample in 2020
T = Tend - Tstart + 1;

date = mdata(Tstart:Tend, 1); % 1930-2020
...
yieldspr = ynom10 - ynom1; % spread between 10-yr yield and 1-yr
y0nom_1 = mean(ynom1);
...
maskbreak1 = find(date < break_year);
maskbreak2 = find(date >= break_year);

deltalogtau = [deltalogtau(maskbreak1) - mean(deltalogtau(maskbreak1)); ...
                   deltalogtau(maskbreak2) - mean(deltalogtau(maskbreak2))];
deltalogg = [deltalogg(maskbreak1) - mean(deltalogg(maskbreak1)); ...
                 deltalogg(maskbreak2) - mean(deltalogg(maskbreak2))];

tts = exp(cumsum(deltalogtau));
gts = exp(cumsum(deltalogg));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Read in debt/gdp
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
gdebt = mdata(Tstart:Tend, 17);
gdebt = [mdata0(17); gdebt]';
