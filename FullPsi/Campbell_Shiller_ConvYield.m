...
s = exp(pdT) .* taxrevgdp' - exp(pdG) .* spendgdp' + cyk';
upper = exp(pxbar) * (mean(taxrevgdp - spendgdp)) + mean(cyk);

save('MAT/CS_ConvYield.mat');
...
parfor iloop = 1:nloop
    ...
    s1 = exp(pdT) .* taxrevgdp' - exp(pdG) .* spendgdp' + cyk';
...
f.PaperSize = [6 4];
print(['../figures/cs_cy_', num2str(round(output_rp * 1000))], '-dpdf', '-fillpage');
