clear; close all;

% Step 2: Fix the L0(1:3) from Step 1 and estimate the rest of the L0 L1 parameters 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Load data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
LoadData_Benchmark;
Load_DividendStrips;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Estimation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
option.estimate.initial = 1; % 0. initial estimate; 1. iterate from the previous estimation
option.estimate.iter = 0; % loop in each estimation step: 0 means only showing estimation results
striphorizon = 4000;

options = optimset('DiffMinChange', 1e-2, ...
    'TolX', 1e-6, 'TolFun', 1e-6, 'MaxIter', 1e4, 'MaxFunEval', 1e8, 'Display', 'iter');

load(['MAT/SDF/x_step1.mat'], 'param');
param0 = param;

filename = ['MAT/SDF/x_step2.mat'];

if option.estimate.initial == 0
    % read from the previous step
    param = zeros(39, 1);
    param(1:2) = param0(4:5);
elseif option.estimate.initial == 1
    load(filename, 'param');
end

if option.estimate.iter > 0
    for i = 1:option.estimate.iter
        param_init = param;
        global counts
        counts = 0;
        [param, fval, exitflag, output] = fminsearch('solve_step2', param_init, options, param0, N, T, Psi, Sig, I_pi, I_gdp, I_y1, I_yspr, I_pdm, I_divgrm, divgrpos, ...
            mu_m, k1m, r0_m, A0m, striphorizon, y0nom_1, y0nom_5, pi0, x0, X2t, tau, yielddata, tipstau, tipsdata, dvd, lev, eps2);
        res_fval(i) = fval;
        save(filename, 'param');
    end

end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Fill in Lambda
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

L0 = zeros(N, 1);
L1 = zeros(N, N);

L0(I_pi == 1) = param0(1);
L0(I_yspr == 1) = param0(2);
L0(I_gdp == 1) = param0(3);

i = 1;
L0(I_divgrm == 1) = param(i); i = i + 1;
L0(I_pdm == 1) = param(i); i = i + 1;

L1(I_pi == 1, I_pi == 1) = param(i); i = i + 1;
L1(I_pi == 1, I_y1 == 1) = param(i); i = i + 1;
L1(I_pi == 1, I_yspr == 1) = param(i); i = i + 1;
L1(I_pi == 1, I_gdp == 1) = param(i); i = i + 1;
L1(I_yspr == 1, 1:N) = param(i:i + N - 1); i = i + N;
L1(I_gdp == 1, 1:N) = param(i:i + N - 1); i = i + N;
L1(I_pdm == 1, 1:N) = param(i:i + N - 1); i = i + N;

Model_Summary;
